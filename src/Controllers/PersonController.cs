...
public class PersonController : ControllerBase
{
      ...
      )
    {
        ...
        try
        {
            ...
            result.Ativado = pessoa.Ativado;
            result.contratos = pessoa.contratos;

            _context.Pessoas.Update(result);
            _context.SaveChanges();
        }
        catch (Exception ex)
        {
            ...
            {
                msg = "Erro ao enviar a solicitação de atualização do Id " + ex.Message
             + id + " especifico",
                status = HttpStatusCode.NotFound
            });
        }
